...
class CausalReasoningEngine:
    ...
    def _get_path_mechanisms(self, path_str: str):
        ...
        return mechanisms
    def _updated_direct_path_query(self, original_prompt_data: dict, causal_paths: list, query_type: str):
        # The overall function structure remains the same:
        """
        Revised query to validate and enrich knowledge graph paths using search and advanced reasoning.
        """
        print(f"Found {len(causal_paths)} direct causal paths. Validating and enriching with reasoning and search...")

        # Mechanism extraction remains the same
        all_mechanisms = []
        ...
        # --- NEW PROMPT ---
        prompt = f"""
        You are a world-class materials science research AI. Your task is to act as a scientific peer reviewer for a hypothesis generated from a knowledge graph. You must use your internal knowledge and **actively use your search tool** to find supporting evidence, quantitative data, and mechanistic details from scientific literature.
        ...
        **Your Instructions:**
        ...
        2.  **Synthesize and Explain:** Based on your search findings and internal knowledge, write a comprehensive mechanistic explanation. Do not just repeat the graph data; synthesize it into a coherent scientific narrative. Your explanation should naturally weave together concepts like:
            - Defect chemistry (e.g., vacancies, interstitials, dopants).
            - Electronic structure changes (e.g., Fermi level shifts, band gap modification).
            - Thermodynamic and kinetic factors (e.g., reaction favorability, activation energies, diffusion rates).
            - Structure-property relationships.

        3.  **Provide Quantitative Analysis:** Search for and include relevant quantitative data. This could include typical ranges for:
            - Carrier concentration ($cm^{{-2}}$ or $cm^{{-3}}$).
            - Band gap values ($eV$).
            - Formation energies ($eV$).
            - Reaction temperatures (°C) or pressures.

        4.  **Discuss Alternatives and Nuances:** Briefly mention any alternative or competing mechanisms that could be at play. What are the key assumptions or boundary conditions for this process to work? (e.g., "This process is effective for monolayers but may not apply to bulk material.").

        **Output Format:**

        Provide your answer as a clear, well-structured Markdown document. At the very end, provide the final numerical/categorical answer in a single ```json block.

        ### Research Analysis

        **1. Validation of Hypothesis**
        *(Critique the knowledge graph pathway here. State whether your search confirms it and cite sources. E.g., "The proposed pathway is well-supported by literature. For example, a study by [Author et al., Year] (Source: [URL]) demonstrates...")*

        **2. Detailed Mechanistic Explanation**
        *(Provide your synthesized narrative here. Explain the step-by-step physics and chemistry in prose.)*

        **3. Quantitative Insights**
        *(List the quantitative data you found. E.g., "The expected hole concentration is in the range of $10^{{12}}$ to $10^{{13}}$ $cm^{{-2}}$...")*

        **4. Alternatives and Considerations**
        *(Discuss other possibilities and important context here.)*

        **5. Conclusion and Confidence Score**
        *(Summarize your findings and provide a confidence score from 0.0 to 1.0.)*
        ...
        if response.candidates and response.candidates[0].grounding_metadata:
            ...
            # --- Step 2: Parse citation information, link the answer to the sources ---
            if hasattr(metadata, 'grounding_supports') and sources:
                print("\n✅ Answer's citations and fact-checking:")
                
                # Iterate through each citation support block
                for i, support in enumerate(metadata.grounding_supports):
                    # Get the cited sentence
                    segment_text = support.segment.text
                    print(f"\n[{i+1}] For this sentence: \"{segment_text}\"")
                    print("    It is supported by the following sources:")
                    
                    # Iterate through all source indices that support the sentence
                    for chunk_index in support.grounding_chunk_indices:
                        if chunk_index < len(sources):
                            # Find the source from the list we stored earlier
                            source = sources[chunk_index]
                            print(f"      - Source [{chunk_index}]: {source['title']} ({source['uri']})")

        else:
            print("\nℹ️ Search tool was not used, the model answered based on its own knowledge.")
    ...
    def _direct_path_query(self, original_prompt_data: dict, causal_paths: list, query_type: str):
        ...
        **Output Format:**
        ...
        if response.candidates and response.candidates[0].grounding_metadata:
            ...
            # --- 步骤 2: 解析引证信息，将回答与来源链接 ---
            if hasattr(metadata, 'grounding_supports') and sources:
                print("\n✅ 回答的引证和事实核查 (Citations & Fact-Checking):")
                
                # 遍历每一个引证支持块
                for i, support in enumerate(metadata.grounding_supports):
                    # 获取被引证的句子
                    segment_text = support.segment.text
                    print(f"\n[{i+1}] 对于这句话: \"{segment_text}\"")
                    print("    它由以下来源支持:")
                    ...
                    for chunk_index in support.grounding_chunk_indices:
                        if chunk_index < len(sources):
                            # 从我们之前存储的列表中查找来源
                            source = sources[chunk_index]
                            print(f"      - 来源 [{chunk_index}]: {source['title']} ({source['uri']})")

        else:
            print("\nℹ️ 搜索工具未被使用，模型依靠自身知识库进行了回答。")
        return extract_json_from_response(response.text)
    ...
    def forward_prediction(self, synthesis_inputs: dict):
        ...
        if causal_context_list:
            ...
            return self._updated_direct_path_query(synthesis_inputs, causal_context_list, "forward")
        else:
...
if __name__ == '__main__':
    ...
    json_file = '../outputs/filtered_combined_doping_data.json' # Make sure this file exists
