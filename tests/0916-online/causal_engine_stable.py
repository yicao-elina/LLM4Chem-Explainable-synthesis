...
class CausalReasoningEngine:
    ...
    Qwen LLM to perform constrained causal reasoning for materials science.
    Includes a similarity-based confidence score and a transfer-learning fallback mechanism.
    """
    def __init__(self, json_file_path: str, model_id: str = "qwen-plus", embedding_model: str = 'all-MiniLM-L6-v2'):
    ...
    def _transfer_learning_query(self, original_prompt_data: dict, analogous_context: str, confidence: float, query_type: str, 
        ...
        """
        Performs a query to Qwen by asking it to reason by analogy and show its work.
        """
        ...
        **Output Format:**
        ...
        print("\n" + "="*80)
        print("TRANSFER LEARNING PROMPT SENT TO QWEN:")
        print("="*80)
        print(prompt)
        print("="*80 + "\n")

        print("Querying Qwen with an interpretable transfer learning prompt...")
    ...
    def forward_prediction(self, synthesis_inputs: dict):
        ...
        if causal_context_list:
            ...
            **Task:**
            ...
            """
            print("Querying Qwen with high-confidence constrained prompt...")
            response_text = self._generate_content(prompt)
            return extract_json_from_response(response_text)
...
if __name__ == '__main__':
    ...
    try:
        # Test Qwen API connection first
        print("Testing Qwen API connection...")
        dashscope.api_key = os.getenv("DASHSCOPE_API_KEY") or "sk-05e23c85c27448a0a8d2e0e0f0302779"
        dashscope.base_http_api_url = 'https://dashscope-intl.aliyuncs.com/api/v1'
        ...
        if test_response.status_code == 200:
            ...
            print(f"Test response: {test_response.output.text[:50]}...")
        else:
            print(f"❌ Qwen API Error: {test_response.code} - {test_response.message}")
            exit(1)

        # Initialize the engine with Qwen
