...
class CausalReasoningEngine:
    ...
    def __init__(self, json_file_path: str, model_id: str = "qwen-plus", api_type: str = "qwen",
                ...
                chroma_path: str = './checkpoints/enriched_knowledge_graph',
                collection_name: str = 'causal_relationships'):
        """
        Initialize with enhanced similarity-based reasoning using hierarchical strategy and ChromaDB
        ...
        print(f"Loading sentence transformer model ('{embedding_model}') for query encoding...")
        self.embedding_model = SentenceTransformer(embedding_model)
        
        print(f"Enhanced Causal Reasoning Engine initialized with hierarchical strategy and ChromaDB.")
        ...
        print(f"ChromaDB collection contains {self.chroma_collection.count()} precomputed embeddings.")
    ...
    def _enhanced_similarity_search(self, query_keywords: list, target_nodes: list = None, top_k: int = 5):
        """
        Enhanced similarity search using ChromaDB that finds multiple similar relationships
        """
        ...
        try:
            # Use ChromaDB to find similar relationships
            results = self.chroma_collection.query(
                query_texts=[query_string],
                n_results=min(top_k * 2, 20)  # Get more results to filter
            )
            ...
            for i, (doc, metadata, distance) in enumerate(zip(
                ...
                cause = metadata.get('cause_parameter', 'Unknown')
                effect = metadata.get('effect_on_doping', 'Unknown')
                
                # Create a relationship string similar to graph nodes
                relationship_text = f"{cause} -> {effect}"
                
                similarities.append((relationship_text, similarity, metadata, doc))
    ...
    def _dag_enhanced_query(self, original_prompt_data: dict, causal_paths: list, 
        ...
        # Use mechanisms from ChromaDB if available, otherwise fall back to graph mechanisms
        if hasattr(self, '_current_mechanisms') and self._current_mechanisms:
            formatted_mechanisms = "\n- ".join(self._current_mechanisms)
        else:
            all_mechanisms = []
    ...
    def _robust_path_finding(self, start_keywords: list, end_keywords: list, reverse: bool = False):
        """
        Enhanced path finding with ChromaDB similarity-based strategies
        """
        ...
        # Try ChromaDB similarity-based matching
        print("🔍 No direct paths found, searching for similar relationships in ChromaDB...")
        
        search_keywords = start_keywords if not reverse else end_keywords
        similar_relationships = self._enhanced_similarity_search(search_keywords, top_k=10)
        
        analogous_paths = []
        similarity_scores = []
        mechanisms = []
        
        for relationship_text, similarity_score, metadata, doc in similar_relationships:
            if similarity_score >= self.similarity_threshold:
                # Extract information from metadata
                cause = metadata.get('cause_parameter', 'Unknown')
                effect = metadata.get('effect_on_doping', 'Unknown')
                mechanism = metadata.get('mechanism_quote', '')
                
                # Create path-like representation
                path_text = f"{cause} -> {effect}"
                analogous_paths.append(path_text)
                similarity_scores.append(similarity_score)
                
                # Store mechanism information
                if mechanism:
                    mechanisms.append(f"{cause} → {effect}: {mechanism}")
        
        if analogous_paths:
            avg_similarity = np.mean(similarity_scores)
            # Store mechanisms for later use
            self._current_mechanisms = mechanisms
            return analogous_paths, "chromadb_similarity", avg_similarity
    ...
    def _get_chromadb_mechanisms(self, relationship_ids: list):
        """
        Get mechanism information from ChromaDB for given relationship IDs
        """
        try:
            results = self.chroma_collection.get(ids=relationship_ids)
            mechanisms = []
            ...
            print(f"⚠️ Error getting mechanisms from ChromaDB: {e}")
            return []
...
if __name__ == '__main__':
    json_file = 'outputs/enriched_knowledge_graph.json'
    ...
    try:
        ...
        engine = CausalReasoningEngine(json_file, model_id="gemini-2.0-flash", api_type="gemini")
