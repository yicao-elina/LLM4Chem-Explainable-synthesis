...
class CausalReasoningEngine:
    ...
    def __init__(self, json_file_path: str, model_id: str = "gemini-1.5-pro-latest", embedding_model: str = 'all-MiniLM-L6-v2', config_dir: str = None):
        ...
        # Initialize LLM
        llm = ChatGoogleGenerativeAI(model="gemini-pro-latest", google_api_key="SOhpM3MmErxSV7hgObzRRF3HZPjOJaOZ")

        # Initialize prompt configuration loader
        self.forward_direct_chain = ForwardDirectChain(llm=llm)
        self.forward_transfer_chain = ForwardTransferChain(llm=llm)
        self.inverse_direct_chain = InverseDirectChain(llm=llm)
        self.inverse_transfer_chain = InverseTransferChain(llm=llm)
    ...
    def forward_prediction(self, synthesis_inputs: dict):
        ...
        if causal_context_list:
            ...
            logger.info(f"Initiating forward direct chain for synthesis: {synthesis_inputs}")
            ...
            logger.info(f"Forward direct chain completed with result: {result}")
            return result
            ...
            if similar_node and score > 0.5:  # Confidence threshold
                ...
                if analogous_paths:
                    ...
                    logger.info(f"Initiating forward transfer chain for analogous synthesis: {synthesis_inputs}")
                    ...
                    logger.info(f"Forward transfer chain completed with result: {result}")
    ...
    def inverse_design(self, desired_properties: dict):
        ...
        if causal_context_list:
            ...
            # Use LangChain inverse direct prompt
                        ...
                        logger.info(f"Initiating inverse direct chain for desired properties: {desired_properties}")
                        result = self.inverse_direct_chain.get_result({
                            "desired_properties": json.dumps(desired_properties, indent=2),
                            "causal_paths": formatted_context,
                            "mechanisms": formatted_mechanisms
                        })
                        logger.info(f"Inverse direct chain completed with result: {result}")
                        return result
            ...
            if similar_node and score > 0.5:  # Confidence threshold
                ...
                if analogous_paths:
                    ...
                    # Use LangChain inverse transfer prompt
                                        ...
                                        logger.info(f"Initiating inverse transfer chain for analogous properties: {desired_properties}")
                                        ...
                                        logger.info(f"Inverse transfer chain completed with result: {result}")
                                        return result
