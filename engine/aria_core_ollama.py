...
class ARIACoreOllama:
    ...
    def __init__(
        self,
        kg_file: str = "../KG/outputs/kg_example_2d_doping_enriched.json",
        model: str = "qwen2:7b",
        embedding_model: str = "all-MiniLM-L6-v2",
        similarity_threshold: float = 0.5,
        use_enhanced_kg: bool = False,
        enhanced_kg_file: str = "../KG/outputs/materials_kg_graph_normalized.json"
    ):
        """
        Initialize ARIA Core system.

        Args:
            kg_file: Path to legacy knowledge graph JSON (doping-specific)
            ...
            use_enhanced_kg: Whether to use new PSP-based multi-level KG
            enhanced_kg_file: Path to enhanced PSP knowledge graph
        """
        self.kg_file = Path(kg_file)
        self.model = model
        self.similarity_threshold = similarity_threshold
        self.use_enhanced_kg = use_enhanced_kg
        self.enhanced_kg_file = Path(enhanced_kg_file)

        # Initialize Ollama client
        self.ollama = get_ollama_client(model=model)

        # Load KG (choose between legacy and enhanced)
        if use_enhanced_kg and self.enhanced_kg_file.exists():
            logger.info("Loading enhanced PSP-based knowledge graph...")
            self.kg = self._load_enhanced_kg()
            self.kg_type = "enhanced"
        else:
            logger.info("Loading legacy doping-focused knowledge graph...")
        ...
        logger.info(f"ARIA Core initialized ({self.kg_type} KG): {self.kg.number_of_nodes()} nodes, "
    ...
    def _load_enhanced_kg(self) -> nx.DiGraph:
        """
        Load enhanced PSP-based multi-level knowledge graph.

        Converts node-link format to NetworkX DiGraph with support for:
        - 5 node types: Material, Process, Structure, Property, Method
        - 5 edge types: undergoes_process, causes_structure, leads_to_property, characterized_by, influences
        """
        if not self.enhanced_kg_file.exists():
            raise FileNotFoundError(f"Enhanced KG file not found: {self.enhanced_kg_file}")

        with open(self.enhanced_kg_file, 'r') as f:
            data = json.load(f)

        # Create MultiDiGraph to preserve multiple edges between same nodes
        G = nx.MultiDiGraph()
        ...
        # Create node type index for fast lookup
        self.node_types_index = {
            'Material': [],
            'Process': [],
            'Structure': [],
            'Property': [],
        ...
        for node, attrs in G.nodes(data=True):
            ...
            if node_type in self.node_types_index:
                self.node_types_index[node_type].append(node)

        logger.info(f"Enhanced KG loaded: {len(self.node_types_index['Material'])} materials, "
                   f"{len(self.node_types_index['Process'])} processes, "
                   f"{len(self.node_types_index['Property'])} properties")
    ...
    def _find_psp_paths(
        ...
        """
        Find Process-Structure-Property causal paths in enhanced KG.

        Searches for paths: Material → Process → Structure → Property

        Args:
            source_material: Material name or keywords
            target_property: Property name or keywords
            max_hops: Maximum path length (default 3 for M→P→S→Prop or M→P→Prop)

        Returns:
            List of path dictionaries with confidence scores
        """
        if self.kg_type != "enhanced":
            logger.warning("PSP path finding only works with enhanced KG")
            return []

        # Find material nodes matching query
        material_nodes = []
        for node_id in self.node_types_index.get('Material', []):
            node_attrs = self.kg.nodes[node_id]
            node_name = node_attrs.get('name', '').lower()
            if any(keyword.lower() in node_name for keyword in source_material.split()):
                material_nodes.append(node_id)

        # Find property nodes matching query
        property_nodes = []
        for node_id in self.node_types_index.get('Property', []):
            node_attrs = self.kg.nodes[node_id]
            prop_name = node_attrs.get('property_name', node_attrs.get('name', '')).lower()
            if any(keyword.lower() in prop_name for keyword in target_property.split()):
                property_nodes.append(node_id)

        if not material_nodes or not property_nodes:
            logger.info(f"No matching nodes found: materials={len(material_nodes)}, properties={len(property_nodes)}")
            return []

        # Find paths between materials and properties
        all_paths = []
        for mat_node in material_nodes:
            for prop_node in property_nodes:
                try:
                    ...
                    for path in paths[:5]:  # Limit to top 5 paths per pair
                        ...
                        # Collect edge info and compute confidence
                        for i in range(len(path) - 1):
                            src, tgt = path[i], path[i + 1]
                            # Get all edges between these nodes
                            edges_data = self.kg.get_edge_data(src, tgt)
                            if edges_data:
                                # Take first edge (or could aggregate)
                                edge_data = list(edges_data.values())[0]
                                path_info['edges'].append({
                                    'type': edge_data.get('type'),
                                    'confidence': edge_data.get('confidence', 1.0),
                                    'evidence': edge_data.get('evidence', [])[:1]  # First evidence quote
                                })
                                # Multiply confidences
                                path_info['confidence'] *= edge_data.get('confidence', 1.0)
        ...
        logger.info(f"Found {len(all_paths)} PSP paths from '{source_material}' to '{target_property}'")
...
def main():
    ...
    try:
        aria = ARIACoreOllama(
            kg_file="../KG/outputs/kg_example_2d_doping_enriched.json",
            model="qwen2:7b"
        )
